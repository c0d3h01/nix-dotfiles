...
case "${1:-}" in
  --enable)  enable=true;  value=3; state="on" ;;
  --disable) enable=false; value=2; state="off" ;;
  *) echo "Usage: $0 --enable | --disable"; exit 1 ;;
esac

command -v iw >/dev/null || { echo "'iw' not found."; exit 1; }

interfaces=($(iw dev | awk '/Interface/ {print $2}'))
[[ ${#interfaces[@]} -eq 0 ]] && { echo "No Wi-Fi interfaces found."; exit 0; }
